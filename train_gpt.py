...
class Hyperparameters:
    ...
    ve_layers = os.environ.get("VE_LAYERS", "5,6")
...
# MLP and attention weights get int6 (6-bit, range -32..31) for better compression.
# Embeddings stay int8 (more sensitive to rounding). Small/control tensors pass through.
...
def generate_autoregressive_calib(model, device, num_seqs=64, seq_len=2048,
    ...
    """Generate sequences autoregressively for GPTQ calibration. No external data."""
    model.eval()
...
def collect_hessians_from_tokens(model, token_seqs, device):
    """Collect H = X^T X from token sequences for GPTQ."""
    hessians = {}
    hooks = []
...
def quantize_int6_per_row(t: Tensor, hessian: Tensor | None = None) -> tuple[Tensor, Tensor]:
    """Int6 quantization with optional Full Hessian GPTQ (Cholesky error compensation)."""
    t32 = t.float()
    ...
    for pct in [0.9995, 0.99999, 1.0]:
        ...
        for i1 in range(0, cols, block_size):
            ...
            for i in range(count):
                w = W1[:, i]
                d = Hinv1[i, i]
                q = torch.clamp(torch.round(w / sf), -32, 31).to(torch.int8)
                Q1[:, i] = q
                err = (w - q.float() * sf) / d
...
def _quantize_int6_simple(t32: Tensor) -> tuple[Tensor, Tensor]:
    """Fallback: simple per-row int6 with percentile search."""
    if t32.ndim == 2:
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            q, s = quantize_int6_per_row(t, hessian=h)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self._q_end = dim
        self._k_end = dim + kv_dim
        self.proj = CastedLinear(dim, dim, bias=False)
        self.proj._zero_init = True
        self.q_gain = nn.Parameter(torch.full((num_heads,), qk_gain_init, dtype=torch.float32))
        self.rotary = Rotary(self.head_dim, base=rope_base, rope_dims=rope_dims)
        self.use_xsa = False  # Set to True for last N layers by GPT.__init__

    def _xsa_efficient(self, y: Tensor, v: Tensor) -> Tensor:
        """Subtract self-value projection so attention focuses on context, not self."""
        B, T, H, D = y.shape
        Hkv = v.size(-2)
        group = H // Hkv
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        qkv = self.c_qkv(x)
        q = qkv[..., :self._q_end].reshape(bsz, seqlen, self.num_heads, self.head_dim).transpose(1, 2)
        k = qkv[..., self._q_end:self._k_end].reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
        v_flat = qkv[..., self._k_end:]
        ...
        # XSA: subtract self-value projection in deep layers
        if self.use_xsa:
            y = y.transpose(1, 2)  # [B, T, H, D]
            v_for_xsa = v.transpose(1, 2)  # [B, T, Hkv, D]
...
class SmearGate(nn.Module):
    """Learned gate that blends each token with its predecessor — helps with local patterns."""
...
class BigramHashEmbedding(nn.Module):
    """Hash table of common letter-pair patterns baked into the model."""
...
class Block(nn.Module):
    def __init__(
        ...
        # LN Scale: damp deeper layers' contributions for stability
        self.ln_scale_factor = 1.0 / math.sqrt(layer_idx + 1) if ln_scale else 1.0

    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(x) * self.ln_scale_factor, v_embed=v_embed)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Value Embeddings — reinject token identity into value projections at specific layers
        ...
        # Enable XSA on the last N layers
        if xsa_last_n > 0:
    ...
    def _get_ve(self, layer_idx: int, input_ids: Tensor, ve_cache: dict) -> Tensor | None:
        if self.ve_shared is None or layer_idx not in self.ve_layer_indices:
            return None
        if "base" not in ve_cache:
            ve_cache["base"] = self.ve_shared(input_ids)
        idx = self.ve_layer_indices.index(layer_idx)
        return ve_cache["base"] * self.ve_layer_scales[idx].to(dtype=ve_cache["base"].dtype)
...
def main() -> None:
    ...
    _accum_total = int(os.environ.get("GRAD_ACCUM_TOTAL", "4"))
    if _accum_total % world_size != 0:
        raise ValueError(f"WORLD_SIZE={world_size} must divide GRAD_ACCUM_TOTAL={_accum_total}")
    grad_accum_steps = _accum_total // world_size
    ...
    if base_model.bigram is not None:
        ...
        # BigramHash embedding and projection go into matrix/scalar as appropriate
        for name, p in base_model.bigram.named_parameters():
            if p is base_model.bigram.scale:
                continue
            if p.ndim == 2:
    ...
    if base_model.ve_shared is not None:
        ...
        for name, p in base_model.ve_shared.named_parameters():
            if p is base_model.ve_shared.scale:
                continue
            if p.ndim == 2:
                matrix_params.append(p)
            else:
    ...
    if args.warmup_steps > 0:
        ...
        # EMA (Exponential Moving Average) — smoothed copy of weights
        ema_state = None
    if args.ema_enabled:
    ...
    while True:
        ...
        skip_eval = int(os.environ.get("SKIP_EVAL", "0"))
        should_validate = (not skip_eval and last_step) or (args.val_loss_every > 0 and step % args.val_loss_every == 0)
        if should_validate:
            torch.cuda.synchronize()
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_flag:
            CastedLinear._qat_flag = True
        ...
        # EMA update: smoothed_weight = decay * smoothed_weight + (1-decay) * current_weight
    ...
    if ema_state is not None:
        log0("ema:applying EMA weights for export")
